...
// Fetch discussions via our Next.js API route (which fetches server-side to avoid CORS)
export async function fetchDiscussions(limit = 5) {
  try {
    ...
    if (!response.ok) {
      console.warn('Discussions API returned:', response.status);
      return null;
    }
...
// Legacy GraphQL function - keeping for reference but requires authentication
export async function fetchDiscussionsGraphQL(limit = 5) {
  try {
    const query = `
      query {
    ...
    return discussions.map(d => ({
      ...
      updated: d.updatedAt,
      body: d.bodyText?.substring(0, 150) + (d.bodyText?.length > 150 ? '...' : ''),
      category: d.category?.name || 'General',
    }));
  } catch (error) {
    console.error('Error fetching discussions from GraphQL:', error);
    return null;
  }
}

// Legacy RSS function - keeping for reference but won't work due to CORS
